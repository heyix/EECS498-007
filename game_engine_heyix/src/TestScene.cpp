...
void FlatPhysics::GenerateHugeTestCaseSkewedCorner(FlatPhysics::DistributedDomain& domain, const FlatPhysics::FlatAABB& world_bounds, int grid_x, int grid_y, int num_bodies, float p_polygon)
{
    float box_half_w = 0.2f;
    float box_half_h = 0.2f;
    float circle_radius = 0.2f;
    ...
    for (int i = 0; i < num_bodies; ++i) {
        ...
        // --- DIFFERENCE FROM ORIGINAL: skewed placement ---

        // First compute normalized grid coords in [0,1]
        ...
        // Skew toward 0 (bottom-left corner) by raising to a power
        ...
        // Everything below is IDENTICAL to your original generator
        x = std::min(std::max(x, world_bounds.min.x()), world_bounds.max.x());
        y = std::min(std::max(y, world_bounds.min.y()), world_bounds.max.y());
