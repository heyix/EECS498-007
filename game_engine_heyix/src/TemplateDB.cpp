			...
			if (it->value.IsObject()) {
				for (auto compIt = it->value.MemberBegin(); compIt != it->value.MemberEnd(); ++compIt) {
					...
					if (compIt->value.IsString()) {
						ComponentDB::Inject_Component_Key_Value_Pair(*new_component, field_key, compIt->value.GetString());
					}
					else if (compIt->value.IsInt()) { 
						ComponentDB::Inject_Component_Key_Value_Pair(*new_component, field_key, compIt->value.GetInt());
					}
					else if (compIt->value.IsFloat()) {
						ComponentDB::Inject_Component_Key_Value_Pair(*new_component, field_key, compIt->value.GetFloat());
					}
					else if (compIt->value.IsBool()) {
						ComponentDB::Inject_Component_Key_Value_Pair(*new_component, field_key, compIt->value.GetBool());
					}
				}
			}
		}
...
//void TemplateDB::Load_Template_Actor(Actor& new_actor,const std::string& template_name)
//{
//	if (template_files.find(template_name) == template_files.end()) {
//		if (!EngineUtils::Resource_File_Exist(folder_path + template_name + ".template")) {
//			std::cout << "error: template " << template_name + " is missing";
...
//}
//
//void TemplateDB::Load_Template_Player(Player& player_template, const std::string& template_name)
//{
//	if (!EngineUtils::Resource_File_Exist(folder_path + template_name + ".template")) {
//		std::cout << "error: template " << template_name << " is missing";
//		exit(0);
