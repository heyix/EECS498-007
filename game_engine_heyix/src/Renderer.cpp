...
void Renderer::draw_text(const std::string& font_name, const std::string& text_content, int font_size, SDL_Color font_color, float x, float y, float zoom_factor)
{
	text_draw_request_queue.push_back(TextDrawRequest(font_name, text_content, font_size, font_color, x, y,zoom_factor));
}


void Renderer::draw_image(const std::string& image_name, const SDL_FRect* src_rect, const SDL_FRect* dest_rect, float zoom_factor)
{
	image_draw_request_queue.push_back(ImageDrawRequest(image_name, src_rect, dest_rect,zoom_factor));
}

void Renderer::draw_image(int actor_id, const std::string& actor_name, const std::string& image_name, const SDL_FRect* src_rect, const SDL_FRect* dest_rect, const float angle, const SDL_FPoint* center, const SDL_RendererFlip& flip, float zoom_factor)
{
	image_draw_request_queue.push_back(ImageDrawRequest(actor_id,actor_name,image_name, src_rect, dest_rect, angle, center, flip,zoom_factor));
}
...
void Renderer::RenderAndClearAllImageRequests()
{
	while (!image_draw_request_queue.empty()) {
		...
		if (image_request.zoom_factor != last_zoom_factor) {
			SDL_RenderSetScale(sdl_renderer, image_request.zoom_factor, image_request.zoom_factor);
			last_zoom_factor = image_request.zoom_factor;
		}
		...
		if (!image_request.render_actor) {
			...
			Helper::SDL_RenderCopyEx(image_request.actor_id, image_request.actor_name,sdl_renderer, texture, get_pointer(image_request.src_rect), get_pointer(image_request.dest_rect), image_request.angle, get_pointer(image_request.center), image_request.flip);
		}
...
void Renderer::RenderAndClearAllFRectRequest()
{
